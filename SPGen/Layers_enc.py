...
class AddNorm(tf.keras.layers.Layer):
    ...
    def __init__(self, name=None, **kwargs):
        ...
        self.ln = LayerNormalization(center=False, scale = False, **kwargs)
        self.add = Add()
...
class myLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        cond_to_multiply = y_true < 0.3
        y_true = tf.where(cond_to_multiply, y_true * 1.5, y_true)
        y_pred = tf.where(cond_to_multiply, y_pred * 1.5, y_pred) 
        
        cond_to_multiply = y_true < 0.6
        y_true = tf.where(cond_to_multiply, y_true * 1.5, y_true) # la première sera multiplié donc par 2.25
        y_pred = tf.where(cond_to_multiply, y_pred * 1.5, y_pred) 
...
class myMse(tf.keras.losses.Loss):
    def __init__(self, **kwargs):
        super(myMse, self).__init__(**kwargs)
        self.mse_loss  = tf.keras.losses.MeanSquaredError()
        #self.msle_loss = tf.keras.losses.MeanSquaredLogarithmicError()
        self.mae_loss  = tf.keras.losses.MeanSquaredError() 
    ...
    def call(self, y_true, y_pred):
        ...
        diff_mean = self.mae_loss(y_true,y_pred)
        # combine the mse and msle and penalty terms
        loss = mse + 0.2*diff_mean
        return loss

    def get_config(self):
        config = super(myMse, self).get_config()
        return config
...
class GenEnc(tf.keras.layers.Layer):
    def __init__(self, units,l1=0,l2=0, **kwargs):
        ...
        self.L1 = Dense(units, activation='swish', name='L1', kernel_regularizer= regularizers.L1L2(l1=l1,l2=l2))
        self.L2 = Dense(units, activation='swish', name='L2', kernel_regularizer= regularizers.L1L2(l1=l1,l2=l2))
        ...
        self.state_g1_layer = Dense(units, activation='swish', name='Lg1', kernel_regularizer=regularizer)
        self.state_g2_layer = Dense(units, activation='swish', name='Lg2', kernel_regularizer=regularizer)
...
class GenPredictionBlock(tf.keras.layers.Layer):
    def __init__(self, units, l1=0, l2=0, **kwargs):
        ...
        self.pre_layer = Dense(4, activation='swish', kernel_regularizer=regularizers.L1L2(l1=l1,l2=l2))
...
class DistLayer(tf.keras.layers.Layer):
    def __init__(self, units=32, l1=0,l2=0, **kwargs):
        ...
        self.ml = Dense(units, activation='swish', name='ml', kernel_regularizer=regularizers.L1L2(l1=l1,l2=l2))
        self.sl = Dense(units, activation='swish', name='sl', kernel_regularizer=regularizers.L1L2(l1=l1,l2=l2))
...
class EncEnc(tf.keras.layers.Layer):
    def __init__(self, preprocessing_units, l1=0, l2=0, _dtype='float32', **kwargs):
        ...
        self.L1 = Dense(d_k, activation='swish', name='L1', kernel_regularizer=regularizers.L1L2(l1=l1, l2=l2), kernel_constraint=kernel_cst, dtype=_dtype)
        self.L2 = Dense(d_k, activation='swish', name='L2', kernel_regularizer=regularizers.L1L2(l1=l1, l2=l2), kernel_constraint=kernel_cst, dtype=_dtype)
        
        self.noise   = GaussianNoise(1e-3,name='noise')
...
class EncDistLayer(tf.keras.layers.Layer):
    def __init__(self, units=32, regularizer=None, outputs=None, _dtype='float32', **kwargs):
        ...
        self.ml = Dense(int(units/2), activation='swish', name='ml',
                        kernel_regularizer=regularizer,
                        dtype=_dtype)
        self.sl = Dense(int(units/2), activation='swish', name='sl',
                        kernel_regularizer=regularizer,
                        dtype=_dtype)
