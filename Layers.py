...
class myLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        # penalty = tf.math.reduce_mean(1.5*diff_min+diff_max)+diff_mean
        penalty = diff_mean
        # combine the mse and msle and penalty terms
        loss = msle + 0.2*penalty
        return loss
...
class EncEnc(tf.keras.layers.Layer):
    def __init__(self, preprocessing_units, l1=0, l2=0, _dtype='float32', **kwargs):
        ...
        self.noise   = GaussianNoise(0.015,name='noise')
